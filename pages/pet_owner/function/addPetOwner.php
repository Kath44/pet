...
if (isset($_POST['add'])) {
    ...
    if (!empty($_POST['pet_owner_name']) && !empty($_POST['pet_owner_contact']) && !empty($_POST['pet_owner_email']) && !empty($_POST['pet_owner_address']) && !empty($_FILES['pet_owner_profile']['name'])) {
        ...
        // Check if pet owner with the same email already exists
        $stmt_check_duplicate = $conn->prepare("SELECT pet_owner_id FROM tbl_pet_owner WHERE pet_owner_email = ?");
        $stmt_check_duplicate->bind_param("s", $petOwnerEmail);
        $stmt_check_duplicate->execute();
        $stmt_check_duplicate->store_result();
        if ($stmt_check_duplicate->num_rows > 0) {
            $_SESSION['error'] = 'A pet owner with the same email already exists';
            ...
            if ($_FILES["pet_owner_profile"]["size"] > 10000000) {
                ...
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                    ...
                    if (move_uploaded_file($_FILES["pet_owner_profile"]["tmp_name"], $targetDirectory . $imageName)) {
                        ...
                        $sql_insert_pet_owner = "INSERT INTO tbl_pet_owner (pet_owner_name, pet_owner_contact, pet_owner_email, pet_owner_address, pet_owner_profile) VALUES (?, ?, ?, ?, ?)";
                        $stmt_insert_pet_owner = $conn->prepare($sql_insert_pet_owner);
                        $stmt_insert_pet_owner->bind_param("sssss", $petOwnerName, $petOwnerContact, $petOwnerEmail, $petOwnerAddress, $imageName);

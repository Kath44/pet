...
if (isset($_POST['add'])) {
    ...
    if (!empty($_POST['adopter_name']) && !empty($_POST['adopter_contact']) && !empty($_POST['adopter_email']) && !empty($_POST['adopter_address']) && !empty($_FILES['adopter_profile']['name'])) {
        ...
        // Check if adopter with the same email already exists
        $stmt_check_duplicate = $conn->prepare("SELECT adopter_id FROM tbl_adopter WHERE adopter_email = ?");
        $stmt_check_duplicate->bind_param("s", $petOwnerEmail);
        $stmt_check_duplicate->execute();
        $stmt_check_duplicate->store_result();
        if ($stmt_check_duplicate->num_rows > 0) {
            $_SESSION['error'] = 'An adopter with the same email already exists';
            ...
            if ($_FILES["adopter_profile"]["size"] > 10000000) {
                ...
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                    ...
                    if (move_uploaded_file($_FILES["adopter_profile"]["tmp_name"], $targetDirectory . $imageName)) {
                        ...
                        $sql_insert_adopter = "INSERT INTO tbl_adopter (adopter_name, adopter_contact, adopter_email, adopter_address, adopter_profile) VALUES (?, ?, ?, ?, ?)";
                        $stmt_insert_adopter = $conn->prepare($sql_insert_adopter);
                        $stmt_insert_adopter->bind_param("sssss", $adopterName, $petOwnerContact, $petOwnerEmail, $petOwnerAddress, $imageName);
